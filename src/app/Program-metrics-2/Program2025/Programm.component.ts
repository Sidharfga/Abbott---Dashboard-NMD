...
export class ProgrammComponent implements OnInit {
  ...
  ngOnInit(): void {}
  ngAfterViewInit() {
    this.xls.getDataFromXl(4, 14).then((data) => {
      var datas = data;
      var j = 0;
    ...
    let chart6 = am4core.create('fin61', am4charts.XYChart);
this.xls.getDataFromXl(4, 13).then((data) => {
  this.financeRaw6 = data; // for table display
  this.finance6 = data
    .map((item: any) => {
      ...
      const isEmpty =
        (!val1 || val1 === 'NA' || val1 === 'TBI') &&
        (!val2 || val2 === 'NA' || val2 === 'TBI');
      ...
      return {
        date: item['key'],
        value1: val1 === 'NA' || val1 === 'TBI' ? null : val1,
        value2: val2 === 'NA' || val2 === 'TBI' ? null : val2,
        value3: item['value3'],
        value4: item['value4'],
        value5: item['value5'],
      };
    })
    .filter((item) => item !== null); // Only graph-safe data
  ...
  changeGraph() {
    ...
    this.xls.getDataFromXl2(4, 12).then((data) => {
      this.finance4 = data;
