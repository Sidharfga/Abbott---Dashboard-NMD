...
@Component({
  ...
  styleUrls: ['./program-metrics.component.scss'],
})
export class ProgramMetricsComponent implements OnInit {
  finance: any = [];
  finance1: any = [];
  burndown: any = [];
  programNames: any = [];
  ...
  ngOnInit(): void {}
  ngAfterViewInit() {
    this.xls.getDataFromXl(4, 4).then((data) => {
      var datas = data;
      var j = 0;
    ...
    let chart6 = am4core.create('fin45', am4charts.XYChart);

    this.xls.getDataFromXl(4, 7).then((data) => {
      this.finance6 = data;
  ...
  changeGraph() {
    ...
    // Create chart instance-chart 4
    let chart = am4core.create('chartdiv99', am4charts.XYChart);

    this.xls.getDataFromXl2(3, 2).then((data) => {
      this.finance4 = data;
     ...
     // debugger;
      ...
      const allValuesNull = finance5.every(
        (item) => item.value1 === null && item.value2 === null && item.value3 === null
      );
      ...
      if (allValuesNull) {
        ...
        label.y = am4core.percent(30); // Adjust this percentage to move the label up or down
